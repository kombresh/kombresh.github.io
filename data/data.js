...
window.DATA = {
 "generated_at": "2026-07-29T19:45:17Z",
 ...
 "samples": [
  {
   ...
   "dynamic": {
    ...
    "notes": {
     ...
     "scope_caveat": "逐輪的完整 API 呼叫軌跡沒有留存 —— run 報告只帶最後一輪的 `api_trace_final`。所以下面每一輪的「新增／消失的 API」是就**外圈這一輪實際碰到的 API 名單**（攔截清單、合成回答、LLM 宣告參數、派送例外、呼叫慣例修正、帶字串引數的呼叫）算的，不是完整呼叫序列的差集。長條的高度（呼叫次數）才是完整計數。"
    },
    "other_runs": [],
    "outcome": {
     "assisted": true,
