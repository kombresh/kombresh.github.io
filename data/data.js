 ...
 "samples": [
  {
   ...
   "handoff": {
    ...
    "treated": {
     ...
     "generated": "2026-07-30 06:33:14",
     "harness": {
      "answered_by_emulator": [
       "flsgetvalue2 -> kernel32.FlsGetValue",
       "getcursorpos -> 一個會動的滑鼠游標(profile: mouse_activity)"
      ],
      "blind_spots": [],
      "filled": [],
      "refused_to_llm": []
     ...
     "rounds": [
      {
       ...
       "llm_calls": 6,
       "llm_diagnosis": "The sample crashed with an invalid instruction (UC_ERR_INSN_INVALID) inside bcryptprimitives.ProcessPrng, which the LLM attempted to answer but timed out. The emulator does not implement ProcessPrng, CreateWaitableTimerExW, SetThreadDescription, SetThreadStackGuarantee, or SetWaitableTimer, so all five were synthesised. The crash occurred after ProcessPrng returned, indicating the synthesised return value or register state was invalid. The sample also queried USERNAME (returned empty) and created a high-resolution waitable timer, both consistent with normal initialisation on a real workstation.",
       "llm_seconds": 365.44,
       "n_apis": 229,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
      ...
      },
      {
       ...
       "llm_calls": 11,
       "llm_diagnosis": "The sample exited cleanly (ExitProcess(0)) after querying display metrics (GetSystemMetrics for SM_CXSCREEN/SM_CYSCREEN) and enumerating a device-class registry key. The run is marked inconclusive solely because five APIs the emulator does not implement (ProcessPrng, CreateWaitableTimerExW, SetThreadDescription, SetThreadStackGuarantee, SetWaitableTimer) were answered by the LLM, and one of those LLM calls (ProcessPrng) timed out. No anti-analysis behaviour, crash, or environment check was observed in the trace.",
       "llm_seconds": 371.44,
       "n_apis": 1182,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
  ...
  },
  {
   ...
   "handoff": {
    ...
    "treated": {
     ...
     "final_watchlist": [
      "mscoree.corexitprocess"
     ],
     "generated": "2026-07-30 06:33:19",
     "harness": {
      "answered_by_emulator": [],
      "blind_spots": [],
      "filled": [],
     ...
     "rounds": [
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The sample is a .NET application that initializes the CLR via 〔檔名已移除〕, then calls CorExitProcess to shut down the runtime and exits cleanly with code 0. The emulator synthesised the return value of GetProcAddress for 〔引文已移除〕 (the API is listed under 〔引文已移除〕 in the Outcome), so the sample received a fabricated function pointer. Because the return was synthesised, the run is marked inconclusive regardless of the sample's actual behaviour. No anti-analysis checks, mouse/input polling, or process enumeration were observed in the trace.",
       "llm_seconds": 0.0,
       "n_apis": 17094,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
      ...
      },
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The sample is a .NET application that initializes the CLR via 〔檔名已移除〕, then calls CorExitProcess(0) and ExitProcess(0) — a clean, normal shutdown. The emulator returned synthesized data for CorExitProcess (an API it does not implement), which forced the run to be marked 〔引文已移除〕 regardless of the sample's actual behavior. No anti-analysis checks, environment probes, or suspicious API patterns are present in the trace. The program simply runs and exits cleanly.",
       "llm_seconds": 0.0,
       "n_apis": 17094,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
   ...
   "summary": {
    ...
    "handoff_deep_treated": 17094,
    "handoff_distinct_control": 56,
    "handoff_distinct_treated": 56,
    "handoff_n_watchlist": 8,
    "handoff_profile_applied": false,
    "handoff_reason_control": "no_behaviour_change",
    "handoff_reason_treated": "no_behaviour_change",
    "handoff_watchlist_seeded": false,
    "id": "336e6c68",
    "is_dll": false,
    "kind": "exe",
    ...
    "treated": {
     ...
     "final_trace": {
      ...
      "top": [
       ...
       {
        "api": "ntdll.NtUnmapViewOfSection",
        "count": 1
       }
      ]
     },
     ...
     "rounds": [
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The sample performed anti-debugging checks (IsDebuggerPresent, CheckRemoteDebuggerPresent) and detected a debugger, displaying a MessageBoxW with 〔引文已移除〕 before exiting. The emulator's process list is likely too clean (missing typical user processes like 〔檔名已移除〕, browser, AV), causing the sample to infer analysis environment. No API returned failure; the checks succeeded and returned 〔引文已移除〕.",
       "llm_seconds": 0.0,
       "n_apis": 13,
       "n_arity_fallbacks": 0,
       "n_faults": 19,
     ...
     "rounds": [
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The sample performs time-based checks using GetLocalTime and SystemTimeToFileTime, then compares file times with CompareFileTime. The emulator's blind spots show that GetLocalTime leaves lpSystemTime uninitialized and SystemTimeToFileTime leaves lpFileTime uninitialized, causing the sample to read garbage values. This leads to failed time comparisons, triggering 〔引文已移除〕 MessageBoxA calls and bailout. The sample also loads 〔檔名已移除〕 and resolves FTP functions, suggesting network functionality that may be hindered by the emulator's lack of real network. The repeated GetModuleFileNameA returning 〔引文已移除〕 while checking for desktop shortcuts indicates environment validation. Setting modules_always_exist ensures 〔檔名已移除〕 loads successfully. The primary issue is uninitialized output parameters in time APIs causing logic failures.",
       "llm_seconds": 0.0,
       "n_apis": 58,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
      ...
      },
      {
       ...
       "llm_calls": 3,
       "llm_diagnosis": "The sample checks for specific shortcut files on the Public Desktop and the current user's Desktop (〔檔名已移除〕, 〔檔名已移除〕, and a Chinese-named .lnk). GetFileAttributesA returns FILE_ATTRIBUTES_NOT_FOUND (0xFFFFFFFF) because the emulator's Desktop folders are empty. The sample then shows 〔引文已移除〕/〔引文已移除〕/〔引文已移除〕 message boxes and exits cleanly. This is an environment check: a real workstation would have at least some shortcuts on the Desktop. Adding busy_desktop=true signals that the desktop should appear populated, which should make the emulator synthesize plausible .lnk files for these paths.",
       "llm_seconds": 2.93,
       "n_apis": 58,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
      ...
      },
      {
       ...
       "llm_calls": 4,
       "llm_diagnosis": "The sample downloads two files via FTP, validates them (likely checking for 〔引文已移除〕 magic), and displays 〔引文已移除〕/〔引文已移除〕 MessageBoxes when validation fails. The emulator serves canned content for InternetReadFile (noted in content_we_fabricated), so the downloaded data does not match what the sample expects. This causes the validation to fail, the sample shows error messages, then retries the entire FTP sequence a second time with different parameters (different file path/offsets). After the second failure, it allocates executable memory (VirtualAlloc with PAGE_EXECUTE_READWRITE) and crashes with an invalid instruction, likely attempting to execute the invalid/corrupted downloaded payload. The crash is a direct consequence of the fabricated FTP content.",
       "llm_seconds": 3.46,
       "n_apis": 82,
       "n_arity_fallbacks": 3,
       "n_faults": 32,
     ...
     "rounds": [
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The trace shows only CRT start-up code (GetSystemTimeAsFileTime, GetTickCount, QueryPerformanceCounter, _initterm_e, _initterm, etc.) and then the process exits with code 255. No anti-analysis APIs, no LoadLibrary/GetProcAddress failures, no process enumeration, and no input polling are present. The emulator marked the run 〔引文已移除〕 with reason 〔引文已移除〕, meaning it had to fabricate a return value for at least one API the emulator does not implement. Because the trace stops during C-runtime initialisation, the most likely cause is that a synthesised return from an unimplemented API (e.g., a heap or TLS helper the CRT calls before main) made the CRT abort start-up. Since we cannot change what the emulator fabricates for unimplemented APIs, the only actionable revision is to present a more ordinary workstation profile so that any subsequent environment checks see plausible values.",
       "llm_seconds": 0.0,
       "n_apis": 10,
       "n_arity_fallbacks": 0,
       "n_faults": 12,
      ...
      },
      {
       ...
       "llm_calls": 0,
       "llm_diagnosis": "The trace ends after CRT initialization (_initterm) with no further API calls, and the outcome is 〔引文已移除〕 with exit code 255. This pattern indicates the program likely attempted to load a DLL (via LoadLibrary/GetProcAddress) that the emulator does not have, causing LoadLibrary to return 0. The subsequent GetProcAddress on the null handle returns 0, and the program calls through a null function pointer, crashing with exit code 255 (0xC0000005 / STATUS_ACCESS_VIOLATION). The crash occurs after the trace window, so the failing LoadLibrary is not visible. Setting modules_always_exist=true will make LoadLibrary succeed for missing DLLs, allowing execution to continue.",
       "llm_seconds": 0.0,
       "n_apis": 10,
       "n_arity_fallbacks": 0,
       "n_faults": 12,
     ...
     "rounds": [
      {
       ...
       "llm_calls": 2,
       "llm_diagnosis": "The sample is a PyInstaller boot-strapper. It queries the current process token twice (TokenUser and TokenIntegrityLevel) and then calls ADVAPI32.ConvertSidToStringSidW to turn the returned SIDs into strings. The emulator does not implement ConvertSidToStringSidW, so the call is synthesised and the run is marked inconclusive. No anti-analysis or crash is involved; the sample simply needs that API to continue.",
       "llm_seconds": 2.34,
       "n_apis": 325,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
      ...
      },
      {
       ...
       "llm_calls": 6,
       "llm_diagnosis": "The sample is a PyInstaller bootstrapper performing normal start-up: it queries several PyInstaller-specific environment variables, opens the process token, and calls GetTokenInformation twice (first to get the required buffer size, then to retrieve the TokenUser SID). The LLM handler for GetTokenInformation incorrectly treated argument 3 (a length/flag value) as an output buffer, causing a plan error. The LLM handler for ConvertSidToStringSidW was invoked but timed out / failed, so the emulator synthesised a return value; because that return is synthetic the run is marked inconclusive. No anti-analysis behaviour is present — the program would exit cleanly (exit code 0) if the two LLM-handled APIs returned correct data.",
       "llm_seconds": 367.45,
       "n_apis": 320,
       "n_arity_fallbacks": 0,
       "n_faults": 32,
       ...
       "warnings": [
        {
         "kind": "warn",
         "text": "這一輪比上一輪**少** 5 次呼叫。迭代之間不是單調遞增，所以圖表用每輪各自獨立的長條，不做累積堆疊。"
        },
        {
         "kind": "warn",
         "text": "這一輪外圈主動「推」了樣本一把（nudged）—— 有一個值是規則塞的，不是模型要的。"
